...
import { Container, Title, Text, Box, SimpleGrid, Stack, Button, Divider, rem, Group } from '@mantine/core'
    ...
    <Box bg="white" style={{ minHeight: '100vh', color: '#111' }}>
        ...
        <Container size={1100}>
          <Stack gap="xl">
            <Box style={{ maxWidth: 800 }}>
              <Text size="xs" fw={700} style={{ letterSpacing: '0.2em', textTransform: 'uppercase' }} c="dimmed">
                The Client Journey
              </Text>
              <Title order={1} mt="md" style={{ fontSize: rem(56), fontWeight: 400, letterSpacing: '-0.02em', lineHeight: 1.1 }}>
                A guided path from <br />stalled draft to submission.
              </Title>
              <Text size="lg" mt="xl" c="dimmed" lh={1.6} style={{ fontSize: rem(20) }}>
                Our process is designed to reduce the inherent uncertainty of independent research by providing clear structure, regular milestones, and expert oversight.
              </Text>
            </Box>
          </Stack>
        </Container>
      ...
      <Box component="section" py={SECTION_SPACING} bg="#F9F8F6" style={{ borderTop: '1px solid #eee' }}>
        <Container size={1100}>
          <Stack gap={rem(40)}>
             {steps.map((item, i) => (
               <Box key={i} py={rem(60)} style={{ borderBottom: i === steps.length - 1 ? 'none' : '1px solid #eee' }}>
                 <SimpleGrid cols={{ base: 1, md: 3 }} spacing="xl">
                    <Text style={{ fontFamily: 'var(--font-serif)', fontSize: rem(48), opacity: 0.2, lineHeight: 1 }}>{item.step}</Text>
                    <Box style={{ gridColumn: 'span 2' }}>
                      <Stack gap="md">
                        <Title order={2} style={{ fontSize: rem(32), fontWeight: 400, fontFamily: 'var(--font-serif)' }}>{item.title}</Title>
                        <Text size="lg" lh={1.7} c="dimmed" style={{ maxWidth: 600 }}>{item.desc}</Text>
                      </Stack>
                    </Box>
                 </SimpleGrid>
               </Box>
      ...
      <Box component="section" py={SECTION_SPACING}>
        <Container size={760}>
          <Stack gap="xl" align="center" style={{ textAlign: 'center' }}>
            <Title order={2} style={{ fontSize: rem(42), fontWeight: 400 }}>What happens next?</Title>
            <Text size="lg" c="dimmed" lh={1.6}>
              After you book a consultation, you will receive a brief intake questionnaire. During the call, we will discuss your project in detail and provide a preliminary assessment of how we can help.
            </Text>
            <Group gap="md">
               <Link href="/scholarcrafted/consultation" style={{ textDecoration: 'none' }}>
                 <Button size="lg" variant="filled" color="dark.9" radius={0}>BOOK A CONSULTATION</Button>
               </Link>
               <Link href="/scholarcrafted/about" style={{ textDecoration: 'none' }}>
                 <Button size="lg" variant="outline" color="dark.9" radius={0}>LEARN ABOUT THE TEAM</Button>
               </Link>
            </Group>
          </Stack>
        </Container>
