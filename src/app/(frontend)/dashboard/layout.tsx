...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isProjectContext =
    pathSegments.length >= 2 && !['portfolio', 'cv'].includes(pathSegments[1])
  ...
  if (!isDashboardHome) {
    ...
    if (isProjectContext) {
      ...
      if (isProjectDetailHome) {
        breadcrumbItems.push(
          <Text key="project" size="sm" ff="var(--font-body)" fw={700}>
        ...
        breadcrumbItems.push(
          <Anchor
            component={Link}
            href={`/dashboard/${activeProjectId}`}
            key="project-link"
            size="sm"
            c="dimmed"
            ff="var(--font-body)"
        ...
        breadcrumbItems.push(
          <Text key="subpage" size="sm" ff="var(--font-body)" fw={700}>
            {breadcrumbNameMap[subPage] || subPage.toUpperCase()}
          </Text>,
        )
      }
    } else {
      ...
      breadcrumbItems.push(
        <Text key="global" size="sm" ff="var(--font-body)" fw={700}>
          {breadcrumbNameMap[globalPage] || globalPage.toUpperCase()}
        </Text>,
      )
    }
  }
      ...
      <NavLink
        ...
        styles={{
          root: {
            ...
            borderRadius: 0,
            color: active
              ? 'var(--mantine-color-burnished-gold-7)'
              : 'var(--mantine-color-deep-green-9)',
          ...
          >
                ...
                <Group gap="md">
                  ...
                  <Group gap={6}>
                    ...
                    <Text ff="var(--font-body)" size="xs" fw={700} c="deep-green.9">
                      {activeProjectId?.toUpperCase() || 'NO_PROJECT_SELECTED'}
                    </Text>
                  </Group>
                </Group>{' '}
              </Box>
            ...
            <Group gap="md" ml="auto">
              <Group gap="xs" visibleFrom="md">
                ...
                >
                  LAST_LOGIN:{' '}
                  <Text component="span" c="deep-green.9" inherit>
                    25_FEB_2026
                  </Text>{' '}
                  {' // '} SESSION_ID:{' '}
                  <Text component="span" c="deep-green.9" inherit>
            ...
            {isProjectContext && activeProjectId && (
              <>
                ...
                <NavItem
                  ...
                  icon={IconFileText}
                  active={pathname === `/dashboard/${activeProjectId}`}
                />
                <NavItem
                  href={`/dashboard/${activeProjectId}/correspondence`}
                  label="Correspondence"
                  icon={IconMail}
                  active={pathname === `/dashboard/${activeProjectId}/correspondence`}
                />
                <NavItem
                  href={`/dashboard/${activeProjectId}/history`}
                  label="Production Trail"
                  icon={IconHistory}
                  active={pathname === `/dashboard/${activeProjectId}/history`}
                />
                <NavItem
                  href={`/dashboard/${activeProjectId}/vault`}
                  label="Asset Vault"
                  icon={IconCertificate}
                  active={pathname === `/dashboard/${activeProjectId}/vault`}
                />
                <NavItem
                  href={`/dashboard/${activeProjectId}/validation`}
                  label="Quality Audit"
                  icon={IconShieldCheck}
                  active={pathname === `/dashboard/${activeProjectId}/validation`}
                />
              </>
            )}
        ...
        <Box p={{ base: 20, sm: 40, lg: 60 }} style={{ flex: 1 }}>
          {breadcrumbItems.length > 0 && (
            <Breadcrumbs
              ...
              separatorMargin="md"
              mb="xl"
              styles={{ separator: { color: 'var(--mantine-color-gray-4)' } }}
            >
              {breadcrumbItems}
            </Breadcrumbs>
          )}
