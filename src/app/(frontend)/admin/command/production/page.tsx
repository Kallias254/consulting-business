...
Preliminary data suggests a significant correlation between...`);
    ...
    <Box style={{ flex: 1, background: '#1a1a1a', display: 'flex', flexDirection: 'column' }}>
      <Box p="xs" bg="#0A0B0C" style={{ borderBottom: '1px solid #2A2D31' }}>
        <Group justify="space-between">
          ...
          <Badge variant="outline" color="burnished-gold">NATURE_MS_v4</Badge>
        </Group>
      </Box>
      
      <ScrollArea flex={1} p={isMobile ? 20 : 40} bg="rgba(0,0,0,0.5)">
        <Center>
          <Paper 
            shadow="xl" 
            radius={0} 
          ...
          >
            ...
            <Stack gap="xl">
              <Box ta="center">
                <Title order={1} style={{ fontSize: isMobile ? '1.2rem' : '1.5rem', color: 'black' }}>
  ...
  return (
    <Box style={{ height: isMobile ? 'auto' : 'calc(100vh - 160px)', display: 'flex', flexDirection: 'column' }}>
      {/* 1. Technical Sub-Header */}
      <Paper withBorder p="md" radius={0} bg="black" mb="xs" style={{ border: '1px solid #2A2D31' }}>
        <Group justify="space-between" wrap="nowrap">
          <Group gap={isMobile ? 'xs' : 'xl'} wrap="nowrap">
            <Group gap="xs" wrap="nowrap">
              <IconTerminal2 size={18} color="var(--mantine-color-burnished-gold-7)" />
              <Text ff="var(--font-body)" size="xs" fw={700} c="#E1E1E1" visibleFrom="sm">ENGINE_STATUS: OPERATIONAL</Text>
              <Text ff="var(--font-body)" size="xs" fw={700} c="#E1E1E1" hiddenFrom="sm">OPERATIONAL</Text>
            </Group>
            <Divider orientation="vertical" h={20} color="#2A2D31" visibleFrom="xs" />
          ...
          <Group gap="md" wrap="nowrap">
            <Badge variant="dot" color="sage" radius={0} size="xs" visibleFrom="sm">WASM_READY</Badge>
            <Group gap="xs">
      ...
      {isMobile ? (
        <Tabs defaultValue="editor" variant="default" radius={0} color="burnished-gold" styles={{ 
          tab: { 
            fontSize: '10px', 
          ...
          panel: { height: '600px', display: 'flex', flexDirection: 'column' }
        }}>
          <Tabs.List style={{ borderBottom: '1px solid #2A2D31', background: '#0A0B0C' }}>
            <Tabs.Tab value="editor" leftSection={<IconCode size={14} />}>Source</Tabs.Tab>
